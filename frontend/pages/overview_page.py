...
class OverviewPage(QWidget):
    def __init__(self, bridge: EngineBridge | None = None) -> None:
        ...
        self.cpu.setMinimumHeight(250)

        cpu_layout.addWidget(self.cpu)

        # ======================
        # CPU PER CORE
        # ======================

        cores_box = QGroupBox("CPU Per Core")

        self.cores_layout = QVBoxLayout(cores_box)
        ...
        self.memory_bar.setMaximumHeight(140)
        ...
        self.c_bar.setMaximumHeight(15)

        self.d_bar.setMaximumHeight(15)
        ...
        self.c_bar.setStyleSheet(disk_style)

        self.d_bar.setStyleSheet(disk_style)

        disk_layout.addWidget(self.c_label)

        disk_layout.addWidget(self.c_bar)

        disk_layout.addWidget(self.d_label)

        disk_layout.addWidget(self.d_bar)
        ...
        self.network_widget.setMaximumHeight(140)
        ...
        load_box = QGroupBox("System Load")
        ...
        load_layout.addWidget(self.load_cpu)

        load_layout.addWidget(self.load_freq)

        load_layout.addWidget(self.load_processes)

        load_layout.addWidget(self.load_threads)

        load_layout.addWidget(self.load_handles)

        # ======================
        # GRID
        # ======================

        grid.addWidget(cpu_box, 0, 0, 1, 2)

        grid.addWidget(cores_box, 0, 2)

        grid.addWidget(memory_box, 1, 0)

        grid.addWidget(disk_box, 1, 1)

        grid.addWidget(network_box, 1, 2)

        grid.addWidget(process_box, 2, 0, 1, 2)

        grid.addWidget(load_box, 2, 2)
    ...
    def update_disks(self):
        ...
        try:
            c = self._bridge.get_disk_usage("C:\\")

            self.c_bar.setValue(int(c["percent"]))
        ...
        try:
            d = self._bridge.get_disk_usage("D:\\")

            self.d_bar.setValue(int(d["percent"]))
            ...
            self.d_label.setText("D: Not Found")
    ...
    def update_system_load(self):
        ...
        self.load_cpu.setText(f"CPU Load: {cpu:.1f}%")

        if freq:
            self.load_freq.setText(f"CPU Frequency: {freq:.0f} MHz")

        else:
            self.load_freq.setText("CPU Frequency: N/A")

        self.load_processes.setText(f"Processes: {processes}")

        self.load_threads.setText(f"Threads: {threads}")

        self.load_handles.setText(f"Handles: {handles}")
    ...
    def update_processes(self):
        ...
        processes.sort(key=lambda x: float(x[2].replace("%", "")), reverse=True)
