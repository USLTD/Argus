...
class PerformancePage(QWidget):
    def __init__(self, bridge: EngineBridge | None = None):
        ...
        cpu_box = QGroupBox("CPU Performance")

        cpu_layout = QVBoxLayout(cpu_box)

        self.cpu_graph = CPUGraph(bridge=self._bridge)

        self.cpu_graph.setMinimumHeight(220)

        cpu_layout.addWidget(self.cpu_graph)

        main.addWidget(cpu_box)
        ...
        self.memory_graph.setMinimumHeight(170)

        memory_layout.addWidget(self.memory_graph)

        middle.addWidget(memory_box)

        network_box = QGroupBox("Network")

        network_layout = QVBoxLayout(network_box)

        self.network_graph = NetworkGraph(bridge=self._bridge)

        self.network_graph.setMinimumHeight(170)

        network_layout.addWidget(self.network_graph)

        middle.addWidget(network_box)
        ...
        disk_box = QGroupBox("Disk")

        disk_layout = QVBoxLayout(disk_box)

        self.disk_graph = DiskGraph("C:\\", bridge=self._bridge)

        self.disk_graph.setMinimumHeight(170)

        disk_layout.addWidget(self.disk_graph)

        bottom.addWidget(disk_box)

        info_box = QGroupBox("CPU Information")
        ...
        cores_box = QGroupBox("CPU Core Usage")
        ...
        for i in range(logical):
            ...
            cores_layout.addWidget(value, row, col + 2)
        ...
        if self._bridge:
            self._bridge.state_updated.connect(self.update_ui)
    ...
    def update_ui(self, ctx: BridgeContext) -> None:
        ...
        self.cpu_usage.setText(f"Usage: {cpu['cpu_percent']:.1f}%")

        freq = cpu["frequency"]

        if freq is None:
            self.cpu_freq.setText("Frequency: N/A")
        else:
            self.cpu_freq.setText(f"Frequency: {freq:.0f} MHz")

        self.cpu_cores.setText(f"Physical Cores: {cpu['physical_cores']}")

        self.cpu_threads.setText(f"Logical Cores: {cpu['logical_cores']}")
        ...
        for i, value in enumerate(per_core):
            ...
            self.core_labels[i].setText(f"{value:.0f}%")
